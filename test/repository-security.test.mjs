...
test('repository pins the approved runtime, lifecycle, and CI supply chain', () => {
  ...
  assert.match(readme, /releases\/v1\.2\.11/u)
  assert.match(staticArtifact, /openSync\(path, constants\.O_RDONLY \| constants\.O_NOFOLLOW\)/u)
  ...
  assert.match(readText('deploy/direct/promote-static-release.sh'), /install_snippet/u)
  assert.match(readText('deploy/direct/promote-static-release.sh'), /restore_snippets/u)
  assert.match(readText('deploy/direct/promote-static-release.sh'), /nginx -T/u)
  assert.match(readText('deploy/direct/promote-static-release.sh'), /verify-nginx-snippet-dump\.sh/u)
  ...
  assert.match(readText('deploy/direct/promote-static-release.sh'), /Cross-Origin-Opener-Policy/u)
  assert.match(readText('deploy/direct/promote-static-release.sh'), /Cross-Origin-Resource-Policy/u)
  assert.match(readText('deploy/direct/promote-static-release.sh'), /Page not found/u)
